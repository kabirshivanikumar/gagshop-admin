      ...
      <div style={{ display: 'grid', gridTemplateColumns: '175px 1fr', gap: 20 }}>
        ...
        <div className="card" style={{ padding: 24 }}>
          ...
          {tab === 'email' && <>
            ...
            <TestEmailButton serviceId={v('emailjs_service_id')} templateId={v('emailjs_template_id')} publicKey={v('emailjs_public_key')} adminEmail={v('admin_email')} toast={toast} />
          </>}
...
// ── Test Email Button (appended) ─────────────────────────────────
function TestEmailButton({ serviceId, templateId, publicKey, adminEmail, toast }) {
  const [testing, setTesting] = useState(false)
  const [result, setResult] = useState(null)

  async function runTest() {
    if (!serviceId || !templateId || !publicKey) {
      setResult({ ok: false, msg: 'Fill in Service ID, Template ID and Public Key first, then save.' })
      return
    }
    setTesting(true)
    setResult(null)
    try {
      ...
      const testParams = {
        to_name: 'Admin',
        to_email: adminEmail || 'test@voidenterprises.xyz',
        order_number: 'TEST-001',
        order_date: new Date().toLocaleDateString(),
        items_list: 'Test Item x1 — $9.99',
        subtotal: '$9.99',
        tax: '$0.00',
        total: '$9.99',
        payment_method: 'Test',
        roblox_username: 'TestPlayer123',
        site_name: 'VoidEnterprises',
        support_email: 'support@voidenterprises.xyz',
        reply_to: adminEmail || '',
      }
      // EmailJS v4: 4th arg must be { publicKey } object
      const r = await emailjs.send(serviceId, templateId, testParams, { publicKey })
      setResult({ ok: true, msg: `✅ Test email sent! Status: ${r.status} — Check ${adminEmail || 'your inbox'}` })
    } catch (err) {
  ...
  return (
    <div style={{ marginTop: 8 }}>
      <button
        ...
        style={{ fontSize: 13 }}
      >
        {testing ? <><div className="spinner spinner-sm" /> Sending test...</> : '📧 Send Test Email'}
      </button>
      {result && (
        <div style={{
